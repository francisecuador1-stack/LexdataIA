...
export function calculatePdVar(input: PdVarInput): PdVarResult {
  ...
  // Caso A/B
  const hasHighRiskTrigger = input.datosSalud || input.datosBiometricos || input.datosMenores
  ...
  const caso = (input.trabajadores === '1-9' || input.trabajadores === '' as never) && !hasHighRiskTrigger ? 'A' as const : 'B' as const;
  ...
  const honorarioMensual = Math.round(base / 50) * 50;
  const implementacionFee = 5400;
